                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control @error('kode') is-invalid @enderror" id="kode" 
                                name="kode" value="{{ old('kode', $pengeluaran->kode) }}" required readonly>
                            @error('kode')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="date" class="form-control @error('tgl_pengeluaran') is-invalid @enderror" id="tgl_pengeluaran" 
                                name="tgl_pengeluaran" value="{{ old('tgl_pengeluaran', $pengeluaran->original_tgl_pengeluaran) }}" required>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control @error('tujuan') is-invalid @enderror" id="tujuan" 
                                name="tujuan" value="{{ old('tujuan', $pengeluaran->tujuan) }}" required>
                            @error('tujuan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-sm-9">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control @error('jumlah') is-invalid @enderror" id="jumlah" 
                                    name="jumlah" value="{{ old('jumlah', $pengeluaran->original_jumlah) }}" required>
                                @error('jumlah')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        // Format awal jumlah saat halaman dimuat
        let initialValue = $('#jumlah').val();
        if (initialValue) {
            initialValue = initialValue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
            $('#jumlah').val(initialValue);
        }
