...
<body>
    ...
    <table>
                ...
                @foreach($pinjaman as $key => $item)
                    <tr>
                        ...
                        <td class="text-center">
                            <span class="{{ $item->status == 'Lunas' ? 'badge-success' : 'badge-warning' }}">
                                {{ $item->status }}
                            </span>
                        </td>
                    </tr>
        ...
        <tfoot>
            ...
            @if(isset($totalBunga) && isset($totalPinjaman))
            <tr>
                <th colspan="3" class="text-right">Total Bunga</th>
                <th class="text-right">{{ number_format($totalBunga, 0, ',', '.') }}</th>
                <th></th>
            </tr>
            <tr>
                <th colspan="3" class="text-right">Total Pengembalian</th>
                <th class="text-right">{{ number_format($totalPinjaman, 0, ',', '.') }}</th>
                <th></th>
            </tr>
            @endif
        </tfoot>
