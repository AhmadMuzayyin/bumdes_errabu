...
<body>
    <div class="header">
        <h2>LAPORAN PENGEMBALIAN PINJAMAN</h2>
        <h3>BUMDES ERRABU</h3>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="30%" class="text-center">Nama Nasabah</th>
                <th width="25%" class="text-center">Jumlah Cicilan (Rp)</th>
            ...
            @if(count($pengembalian) > 0)
                @foreach($pengembalian as $key => $item)
                    <tr>
                        ...
                        <td class="text-center">{{ $item->tgl_pengembalian->format('d/m/Y') }}</td>
                        <td>{{ $item->pinjaman->nasabah->nama }}</td>
                        <td class="text-right">{{ number_format($item->nominal_cicilan, 0, ',', '.') }}</td>
                        <td class="text-center">
                            <span class="{{ $item->pinjaman->status == 'Lunas' ? 'badge-success' : 'badge-warning' }}">
