<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Laporan Nasabah</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            font-size: 12px;
            margin: 0;
            padding: 0;
        }
        .header {
            text-align: center;
            margin-bottom: 20px;
        }
        .header h2, .header h3 {
            margin: 5px 0;
        }
        .info {
            margin-bottom: 15px;
        }
        table {
            width: 100%;
            border-collapse: collapse;
            margin-bottom: 15px;
        }
        table, th, td {
            border: 1px solid #333;
        }
        th, td {
            padding: 5px;
        }
        th {
            background-color: #f2f2f2;
        }
        .text-center {
            text-align: center;
        }
        .text-right {
            text-align: right;
        }
        .footer {
            margin-top: 30px;
            text-align: right;
        }
        .section {
            margin-bottom: 20px;
        }
        .page-break {
            page-break-after: always;
        }
    </style>
</head>
<body>
    <div class="header">
        <h2>LAPORAN TRANSAKSI NASABAH</h2>
        <h3>BUMDES ERRABU</h3>
    </div>
    
    <div class="info">
        <table border="0" style="margin-bottom: 10px">
            <tr>
                <td width="15%"><strong>Periode</strong></td>
                <td>: {{ \Carbon\Carbon::parse($tanggalAwal)->format('d/m/Y') }} - {{ \Carbon\Carbon::parse($tanggalAkhir)->format('d/m/Y') }}</td>
            </tr>
            <tr>
                <td><strong>Nasabah</strong></td>
                <td>: {{ $nasabah->nama }}</td>
            </tr>
            <tr>
                <td><strong>Tanggal Cetak</strong></td>
                <td>: {{ date('d/m/Y') }}</td>
            </tr>
        </table>
    </div>
    
    <div class="section">
        <h3>Transaksi Simpanan</h3>
        <table>
            <thead>
                <tr>
                    <th width="5%">No</th>
                    <th width="15%">Tanggal</th>
                    <th width="10%">Jenis</th>
                    <th width="30%">Nominal</th>
                    <th width="30%">Saldo</th>
                </tr>
            </thead>
            <tbody>
                @if(count($transaksiSimpanan) > 0)
                    @foreach($transaksiSimpanan as $key => $item)
                        <tr>
                            <td class="text-center">{{ $key + 1 }}</td>
                            <td class="text-center">{{ \Carbon\Carbon::parse($item['tanggal'])->format('d/m/Y') }}</td>
                            <td class="text-center">{{ $item['jenis'] }}</td>
                            <td class="text-right">{{ 'Rp ' . number_format($item['nominal'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ 'Rp ' . number_format($item['saldo_berjalan'], 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                @else
                    <tr>
                        <td colspan="5" class="text-center">Tidak ada data simpanan.</td>
                    </tr>
                @endif
            </tbody>
            <tfoot>
                <tr>
                    <th colspan="4" class="text-right">Saldo Akhir</th>
                    <th class="text-right">{{ 'Rp ' . number_format($saldoAkhirSimpanan, 0, ',', '.') }}</th>
                </tr>
            </tfoot>
        </table>
    </div>

    <div class="section">
        <h3>Pinjaman Belum Lunas</h3>
        <table>
            <thead>
                <tr>
                    <th width="5%">No</th>
                    <th width="15%">Tanggal</th>
                    <th width="25%">Total Pinjaman</th>
                    <th width="25%">Total Pengembalian</th>
                    <th width="30%">Sisa Pinjaman</th>
                </tr>
            </thead>
            <tbody>
                @if(count($transaksiPinjaman) > 0)
                    @foreach($transaksiPinjaman as $key => $item)
                        <tr>
                            <td class="text-center">{{ $key + 1 }}</td>
                            <td class="text-center">{{ \Carbon\Carbon::parse($item['tanggal'])->format('d/m/Y') }}</td>
                            <td class="text-right">{{ 'Rp ' . number_format($item['total_pinjaman'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ 'Rp ' . number_format($item['total_pengembalian'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ 'Rp ' . number_format($item['nominal'], 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                @else
                    <tr>
                        <td colspan="5" class="text-center">Tidak ada pinjaman yang belum lunas.</td>
                    </tr>
                @endif
            </tbody>
            <tfoot>
                <tr>
                    <th colspan="4" class="text-right">Total Pinjaman Belum Lunas</th>
                    <th class="text-right">{{ 'Rp ' . number_format($totalPinjaman, 0, ',', '.') }}</th>
                </tr>
            </tfoot>
        </table>
    </div>

    <div class="footer">
        <table border="0">
            <tr>
                <td width="70%"></td>
                <td width="30%" style="text-align: center;">
                    Petugas Simpan Pinjam
                    <br><br><br><br>
                    _______________________
                    <br>
                    BUMDES Errabu
                </td>
            </tr>
        </table>
    </div>
</body>
</html>
